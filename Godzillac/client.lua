RegisterNetEvent("Godzilla:cleanareavehy")
RegisterNetEvent("Godzilla:cleanareapedsy")
RegisterNetEvent("Godzilla:cleanareaentityy")
RegisterNetEvent("Spartan:openmenuy")
RegisterNetEvent("Spartan:adminmenuenabley")
RegisterNetEvent("Spartan:invalid")

local titolo = "~u~Godzilla ~s~Admin Menu"
...
local explosion_spam = 0
local blacklistedspam = 0
local explosionspamtoban = 0
AddEventHandler('entityCreating', function()
  for _,blacklistedentity in ipairs(carsblack) do
    if blacklistedentity == 'chino' then
      explosion_spam = explosion_spam + 1
      if explosion_spam > explosionspamtoban then
	  DeleteVehicle(vehicle)
        print(explosion_spam)
      end
      Citizen.Wait(5000)
      explosion_spam = 0
    end
...
TriggerServerEvent('Spartan:adminmenuenable')

AddEventHandler("Spartan:adminmenuenabley", function()
	Enabled = false
	showblip = false
	showsprite = false
	nameabove = false
...
AddEventHandler("Spartan:invalid", function()
...
Citizen.CreateThread(
	function()
		...
		LR.CreateSubMenu(sMX, SpartanIcS, acexd)
		LR.CreateSubMenu(TRPM, SpartanIcS, acexd)
		LR.CreateSubMenu(advm, SpartanIcS, acexd)
		LR.CreateSubMenu(VMS, SpartanIcS, acexd)
		LR.CreateSubMenu(OPMS, SpartanIcS, acexd)
		LR.CreateSubMenu(poms, OPMS, acexd)
		LR.CreateSubMenu(crds, SpartanIcS, acexd)
		...
		while Enabled do
			if LR.IsMenuOpened(SpartanIcS) then
				TSE('Spartan:checkup')
				DrawTxt("Godzilla AC V2 ~s~- "..pisello, 0.80, 0.9)
				notify("~u~Godzilla AC V2 ", false)
				if LR.MenuButton("~h~~p~#~s~ Online Players", OPMS) then
				elseif LR.MenuButton("~h~~p~#~s~ Vehicle Menu", VMS) then
                elseif LR.MenuButton("~h~~p~#~s~ Server Options", advm) then
				elseif LR.MenuButton("~p~# ~bGodzilla AC", crds) then
								end
	...
	LR.Display()
elseif IsDisabledControlPressed(0, Config.AdminMenuKey)  then
			TSE('Spartan:openmenu')
...
elseif LR.IsMenuOpened(advm) then
	if LR.Button("Clean Area","~g~Vehicles") then
		TSE("Godzilla:cleanareaveh")
	elseif LR.Button("Clean Area","~r~Peds") then
		TSE("Godzilla:cleanareapeds")
	elseif LR.Button("Clean Area","~y~Entity") then
		TSE("Godzilla:cleanareaentity")
    elseif LR.Button("Delete", "~b~Networking Objects") then
        TSE("Godzilla:networkingobjetos")
end

	LR.Display()
	elseif LR.IsMenuOpened(crds) then
		if LR.Button("~h~https://discord.gg/JBZ2JNC") then
	end
RegisterNetEvent("Godzilla:networkingobjetcts2")
AddEventHandler(
    "Godzilla:networkingobjetcts2",
    function()
        objst = 0
	...
	AddEventHandler("Godzilla:cleanareavehy", function()
	...
	AddEventHandler("Godzilla:cleanareapedsy", function()
		PedStatus = 0
	...
	AddEventHandler("Godzilla:cleanareaentityy", function()
		objst = 0
        for db in EnumerateObjects() do
            objst = objst + 1
            DeleteEntity(db)
	...
	AddEventHandler("Spartan:openmenuy", function()
